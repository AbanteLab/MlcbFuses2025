...
### Assembly of hybrid feature matrix with SNPs and VAE encodings ###
...
for gene_file in sorted(os.listdir(embedding_dir)):
    ...
    if os.path.isfile(file_path) and gene_file.endswith('.txt.gz'):
        ...
        if match:
            ...
            min_val = gene_tensor.min()
            max_val = gene_tensor.max()
            normalized_tensor = (gene_tensor - min_val) / (max_val - min_val + 1e-8)  # Add epsilon to prevent division by zero
            
            # Scale the tensor to [0, snp_max] to have same range as snp coded genes
            scaled_tensor = normalized_tensor * snp_max
            
            # Add the scaled tensor to the list (no padding here)
...
for gene in genes_feat_snps['gene']:
    ...
    # Add the gene tensor to the list (no padding here)
    gene_tensors.append(gene_tensor)

    # Append the gene name to the order list, as many times as columns added as features
    for _ in range(len(gene_data)):
        gene_order.append(gene)

# Concatenate all the gene tensors into a single 2D tensor
# Each row is a sample and each column is a feature of all genes concatenated
final_tensor = torch.cat(gene_tensors, dim=-1)

torch.save(final_tensor, f'{feature_dir}2d_gene_tensor_vae_snps.pth')

# Save the gene order to a text file
with open(f'{feature_dir}2d_gene_tensor_vae_snps_order.txt', 'w') as f:
